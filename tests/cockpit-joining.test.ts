...
import { DEMO_TASKS, TAXI_NAMESPACE, isVisitor, joining } from "@/src/features/cockpit/joining";
import type { JoinInput } from "@/src/features/cockpit/joining";
 ...
 * the fixture hid.
 */
function mine(name: string, overrides: Partial<TaskRecord> = {}): TaskRecord {
...
describe("whose task it is: obsel's own work is the closed set", () => {
  ...
  it("counts an agent whose tables landed in obsel's own namespace, which is all of them", () => {
     ...
     */
    expect(isVisitor(mine("clean_expenses"))).toBe(true);
    expect(mine("clean_expenses").writes[0]).toContain("obsel_demo.clean_expenses");
  });
  ...
  it("counts a visitor that reads one of the demo's tables", () => {
    ...
    expect(isVisitor(mine("summarise", { reads: [ds("obsel_demo", "clean_orders")] }))).toBe(true);
  });
  ...
  it("counts a task with no tables at all as a visitor", () => {
    ...
    expect(isVisitor(mine("bare", { reads: [], writes: [] }))).toBe(true);
  });

  it("counts a taxi task as obsel's own even when it only reads the namespace", () => {
    const reader = mine("some_rollup", {
...
describe("the steps tick only on what the board shows", () => {
  ...
  it("ticks registration alone when a visitor has only declared itself", () => {
    const result = view([mine("clean_expenses")]);
  ...
  it("ticks the announcement without the report while work is under way", () => {
    const result = view([mine("clean_expenses", { status: "running", startedAt: AT })]);
    expect(result.steps.map((step) => step.done)).toEqual([true, true, false, false]);
  });

  it("ticks the report once a run has finished", () => {
    const result = view([mine("clean_expenses", { startedAt: AT, finishedAt: AT })]);
  ...
  it("ticks the answer when obsel has flagged the visitor's work", () => {
    const result = view([
      mine("clean_expenses", { startedAt: AT, finishedAt: AT }),
      mine("monthly_totals", { startedAt: AT, finishedAt: AT, stale: mark() }),
    ]);
  ...
  it("spreads the ticks across different agents, because the swarm is the subject", () => {
    ...
    const result = view([mine("clean_expenses"), mine("monthly_totals", { finishedAt: AT })]);
...
describe("when the steps are painted rather than folded away", () => {
  ...
  it("opens while a visiting agent is part way through", () => {
    expect(view([mine("clean_expenses")]).expanded).toBe(true);
  });
  ...
  it("stays folded and ticks nothing when the read failed", () => {
    ...
    const blind = joining({ trusted: false, tasks: [mine("clean_expenses")], command: null });
