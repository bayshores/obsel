...
import { AMBER, ROSE } from "@/src/features/cockpit/backdrop-shader";

/**
 * The backdrop shader is the one place in the cockpit that cannot use a token.
 *
...
describe("quiet text clears WCAG AA on every background it is drawn on", () => {
  ...
  it("records why mmux's own muted token could not be used as-is", () => {
    ...
    // someone "simplifying" the cockpit back onto it.
