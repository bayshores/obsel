/**
 * The bench's pure half: what leaves the browser, and what it says it will do.
 *
 * Nothing here reaches the network or a hash. The fingerprint has exactly one
 * implementation, in `agents/fingerprint.py`, and the bench's job is to hand it
...
import {
  ...
  withRowAdded,
  withRowRemoved,
} from "@/src/features/cockpit/bench";
import type { BenchTable } from "@/src/features/cockpit/bench";

/** A table built the way the panel builds one, so ids are real ids. */
function tableOf(names: string[], rows: string[][]): BenchTable {
...
describe("what leaves the browser", () => {
  ...
  it("keeps a column's values when the column is renamed", () => {
    /*
     * The single most interesting thing the bench can do and the first thing
  ...
  it("sends a number as a number so reformatting it is not a change", () => {
     ...
     * reported a change nobody made. It only sees numbers. A bench that sent
...
describe("what the bench refuses to send", () => {
