...
import styles from "./your-data.module.css";
...
export function YourDataPanel({ view, tasks }: { view: YourDataView; tasks: TaskRecord[] }) {
  /*
  ...
  const [draft, setDraft] = useState<YourDataDraft>(EMPTY_DRAFT);
  ...
  async function register(): Promise<void> {
    ...
    try {
       ...
       *
       * `your-data.ts` paints the form only on a board with nothing on it, so the
  ...
  return (
    <Panel
       ...
       */
      meta={view.yourData.length > 0 ? `${view.yourData.length} of yours` : undefined}
    ...
    >
      ...
      >
        ...
        <div className={styles.body}>
          ...
          {view.yourData.length > 0 && <Yours view={view} />}
 ...
 */
function Yours({ view }: { view: YourDataView }) {
  return (
    <ul className={styles.tasks} aria-label="Your own tables">
      {view.yourData.map((task) => (
...
/**
 * One of your tasks, with the table form folded behind it.
 *
 ...
 * have to scroll past every table form to reach the form.
 *
 * Open state is per task and lives here rather than in `your-data.ts`, because it is
 ...
 */
function Your({ task }: { task: YourDataTask }) {
