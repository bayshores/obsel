...
/**
 * Where the panel sits, how wide it is, and whether it is out of the way.
 *
 ...
 * a first-paint correction that shows the panel arriving on the wrong side.
 */
...
const STORE = "obsel.panel.v1";

/** What the server renders, and what a browser with no stored answer gets. */
const DEFAULTS: PanelPrefs = { side: DEFAULT_SIDE, width: DEFAULT_WIDTH, collapsed: false };
 ...
 */
let snapshot: PanelPrefs | null = null;
 ...
 * panel width.
 */
function restore(): PanelPrefs {
  try {
    const raw = window.localStorage.getItem(STORE);
    if (raw === null) return DEFAULTS;
    const parsed = JSON.parse(raw) as Partial<Record<keyof PanelPrefs, unknown>>;
    return {
...
function getSnapshot(): PanelPrefs {
  if (snapshot === null) snapshot = restore();
  return snapshot;
}

function getServerSnapshot(): PanelPrefs {
  return DEFAULTS;
}
 ...
 */
function commit(next: PanelPrefs): void {
  ...
  try {
    ...
    // A browser refusing storage is not a reason to refuse to move the panel.
  }
...
export function usePanel(): PanelState {
  ...
  const setSide = useCallback((side: PanelSide) => {
  ...
  /*
   * A window narrowed past the clamp takes the panel with it. Without this, a
   * 620px panel stored on a desktop opens on a 900px laptop with 280px left for
