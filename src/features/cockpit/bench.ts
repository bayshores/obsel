/**
 * The bench: a table you edit by hand, and what obsel can honestly say about it
 * before you hand it over.
 *
 * Pure, like `guide.ts`, `joining.ts` and `mine.ts` beside it. Nothing here
 * talks to the network and nothing here hashes anything — `bench-panel.tsx`
 ...
 * single most interesting thing the bench can do, and the first thing anybody
 * tries.
 */
...
export function benchColumn(name: string): BenchColumn {
  return { id: freshId("c"), name, dropped: false };
}

export function benchRow(cells: Record<string, string> = {}): BenchRow {
 ...
 * imposed: `bench-panel.tsx` puts it behind a button labelled as an example.
 ...
 */
export function exampleTable(): BenchTable {
...
export function blankTable(): BenchTable {
...
export function withColumnAdded(table: BenchTable): BenchTable {
  return { ...table, columns: [...table.columns, benchColumn("")] };
}

export function withColumnRenamed(table: BenchTable, id: string, name: string): BenchTable {
  return {
    ...table,
...
export function withColumnDropped(table: BenchTable, id: string, dropped: boolean): BenchTable {
  return {
    ...table,
...
export function withRowAdded(table: BenchTable): BenchTable {
  return { ...table, rows: [...table.rows, benchRow()] };
}

export function withRowRemoved(table: BenchTable, id: string): BenchTable {
  return { ...table, rows: table.rows.filter((row) => row.id !== id) };
}

/** The columns that will actually be sent, in the order they are shown. */
export function livingColumns(table: BenchTable): BenchColumn[] {
 ...
 * sees numbers. Sending every cell as a string would put the bench on the far
 ...
 */
export function payload(table: BenchTable): BenchPayload {
 ...
 * reason as `draftProblem` in `mine.ts`: a bench this small needs the next
 ...
 */
export function tableProblem(table: BenchTable): string | null {
  const columns = livingColumns(table);
  if (columns.length === 0) {
 ...
 * backwards — so the bench does not guess whether the content moved, and says
...
export function benchDiff(table: BenchTable, recorded: RecordedShape | null): BenchDiff | null {
 ...
 *   compares the values, and the bench genuinely does not know the answer until
 *   obsel says.
 *
 ...
 */
export function benchLine(diff: BenchDiff | null): string {
  if (diff === null) {
