...
/**
 * The bench on screen: your table, your hands, obsel's real answer.
 *
 * One of these appears under each of your own tasks in `mine-panel.tsx`. It
 ...
 * back — and a bench that quietly re-seeded blank rows on reload would report a
...
import type { BenchTable, RecordedShape } from "./bench";
...
export function BenchPanel({
   ...
   * typed. The dataset URN is not needed here: `mine.ts` has already looked the
   ...
   */
  const [table, setTable] = useState<BenchTable>(() => seed(recorded));
  ...
  async function report(): Promise<void> {
    ...
    try {
       ...
       * state, which is the same discipline `mine-panel.tsx` and
  ...
  return (
    <div className={styles.bench}>
      ...
      <div className={styles.scroller}>
        <table className={styles.grid}>
          {/* Clipped away rather than absent. It is the accessible name for a
              ...
              screen reader can follow. `bench.module.css` explains the idiom. */}
              ...
              <tr key={row.id}>
                ...
                <td>
                  <button
                    ...
                    // accepts it, but it is never what somebody at the bench
      ...
      <div className={styles.controls}>
        ...
        {/* Offered rather than imposed, and labelled as an example. `bench.ts`
...
/**
 * The table the bench opens with.
 *
 ...
 */
function seed(recorded: RecordedShape | null): BenchTable {
