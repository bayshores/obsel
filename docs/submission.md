...
Any MCP-capable agent can join a swarm through obsel's own MCP server (six tools; every mutation
goes through obsel's HTTP API and the server holds no DataHub credentials). The
