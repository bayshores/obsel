...
Checked against the working tree on 2026-08-10; unit and self-check counts were enumerated that
day, and live counts are the recorded 2026-08-09 run. Treat the shipped column as "present and
...
| The integration harness                     | `tests/live/`, `vitest.live.config.ts`, `tests/support/server-only.ts`                                         | shipped, 176 tests across 16 files, no stand-ins                 |
| URN shapes                                  | `src/server/datahub/urns.ts`                                                                                   | shipped                                                          |
| HTTP API                                    | the seventeen routes under `app/api/`, listed in [section 11](#11-the-http-api)                                | shipped                                                          |
| Page                                        | `app/page.tsx`, `src/features/dashboard/`                                                                      | shipped, covered by the rows below and the browser suite         |
| Live agent progress                         | `src/features/dashboard/progress.ts`                                                                           | shipped, 28 passing tests, seen live                             |
| The guide                                   | `src/features/dashboard/guide/guide.ts`, `guide-panel.tsx`                                                     | shipped, 77 tests across the three guide files, driven live      |
| The joining guide, four steps off the swarm | `src/features/dashboard/joining/joining.ts`, `joining-panel.tsx`                                               | shipped, 25 unit + 38 browser tests                              |
| Human names for tasks and tables            | `src/features/dashboard/naming.ts`, `staleness.ts` (`tableLabel`, `taskLabel`)                                 | shipped, 19 passing tests                                        |
...
| Grouping the trace into decisions           | `src/features/dashboard/trace/passes.ts`                                                                       | shipped, 15 unit tests                                           |
| The lineage graph                           | `src/features/dashboard/graph/lineage.tsx`, `nodes.tsx`, `positions.ts`, `cascade.ts`, on React Flow and dagre | shipped, 34 unit + 48 browser tests                              |
| Naming which columns moved                  | `src/server/coordinator/staleness.ts` (`columnChange`); `obsel.stale.columns`                                  | shipped, covered inside `tests/staleness.test.ts`, verified live |
| Reading the stale tag back onto the page    | `src/server/datahub/tags.ts`, `src/features/dashboard/timing.ts` (`totals`); the ribbon's write-back cell      | shipped, 45 unit tests across the two files                      |
| The link into DataHub's own UI              | `src/features/dashboard/datahub-link.ts`, `details/inspector.tsx`                                              | shipped, 6 tests, path read from DataHub's bundle                |
