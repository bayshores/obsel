...
in `e2e/dashboard.spec.ts` → "the details surface" and two in `e2e/erasure.spec.ts`, and earlier the
...
| a failed read on the page                 | every measured number withheld, `e2e/dashboard.spec.ts` honesty block                                                                                                                                                                                                                                                                                    | browser            |
...
| somebody's own agent part way through     | the four joining steps over seeded pages, `tests/dashboard-joining.test.ts`; painted and folded, `e2e/dashboard.spec.ts` "bring your own agent"; a real MCP session ticking all four, 2026-07-24                                                                                                                                                         | unit, browser, run |
...
| the server stopped, both reads failing    | one fault and one report rather than two sentences that disagree, `e2e/dashboard.spec.ts`; the demo read failing alone still says the page is current, same file; found on a real stopped server 2026-07-27 in `verification.md`                                                                                                                         | browser, run       |
| obsel restarted on a walked page          | reset still offered with an empty launcher history and with no activity read at all, and still withheld from a page that only ran, `tests/dashboard-guide.test.ts`; the same two pages in `e2e/dashboard.spec.ts`; the signal checked against the recorded repaired forty-task pipeline, which carries it on 3 of 40                                     | unit, browser, run |
...
| which page is on screen                   | the header names the flow, and its disclosure names `OBSEL_FLOW_ID` as the way to open a different one, `e2e/dashboard.spec.ts`                                                                                                                                                                                                                          | browser            |
| one action asked for, several offered     | at most one accent per stage across every stage, `tests/dashboard-guide.test.ts`; every action label the same computed size with exactly one colour occurring once, `e2e/dashboard.spec.ts`                                                                                                                                                              | unit, browser      |
...
| ----------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
| neither axis of the page ever scrolls           | `e2e/dashboard.spec.ts` "fit" at both viewports; `e2e/scale.spec.ts` on the 82-node page; re-asserted after every rearrangement in `e2e/dock.spec.ts`                                 | browser       |
...
| the change spreads outward one hop at a time    | `e2e/dashboard.spec.ts`: each lit edge's animation delay is ordered by the hop count obsel recorded, and hop 2 starts after hop 1                                                     | browser       |
| the ripple never replaces the mark it covers    | `e2e/dashboard.spec.ts`: the three marked tasks are amber from `nodeTone` alone, each carries a flare, and no unmarked task carries one                                               | browser       |
| the measured number arrives once and holds      | `tests/dashboard-count-up.test.ts` pins the ends of the curve; `e2e/dashboard.spec.ts` samples the cell twelve times over three seconds and it never leaves the measured value        | unit, browser |
| reduced motion shows the finished picture       | `e2e/dashboard.spec.ts`: the lit path stays drawn with no animations attached, and the detection figure is the measured one on the first frame                                        | browser       |
...
| condition                                         | proven by                                                                                                                                                                                 | kind          |
| ------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------- |
| the page says the boxes respond, before any do    | `e2e/dashboard.spec.ts` "the details surface": the hint is present on a populated page and absent on an empty one, where there is nothing to point at                                     | browser       |
| pointing at a box previews it                     | `e2e/dashboard.spec.ts`: the preview appears on hover, carries the writer and readers, and contains no `urn:li:` anywhere                                                                 | browser       |
| pointing moves nothing in the graph               | `e2e/dashboard.spec.ts`: every node top, the graph's height and the page scroll height are unchanged while a preview is up                                                                | browser       |
| a click pins, Esc unpins, the preview pins itself | `e2e/dashboard.spec.ts`: three tests, one per path into and out of the pinned depth                                                                                                       | browser       |
| what is pinned survives pointing elsewhere        | `e2e/dashboard.spec.ts`: with one node pinned, hovering another leaves the panel naming the pinned one                                                                                    | browser       |
| the panel names its subject exactly once          | `e2e/dashboard.spec.ts`: counts elements whose whole text is the heading, inside the panel. It was three before: title, kind line, heading                                                | browser       |
| a table is sketched from its reported shape       | `e2e/dashboard.spec.ts`: real column names render, `+ order_total_usd` and `- order_total` agree with the mark on the same page, and the caption states the counts and the derivation     | browser       |
| the sketch cannot show a value                    | `e2e/dashboard.spec.ts`: every placeholder block is empty. `Schematic` is passed a column list and a row count, so there is no path by which one could be filled                          | browser       |
| a writer that reported nothing says so            | `e2e/dashboard.spec.ts` on the registered-task fixture: the plain sentence, and no sketch at all                                                                                          | browser       |
...
| the flow highlight lights exactly what it touches | `tests/dashboard-flow.test.ts` (writer, every reader, one hop only, unknown urn empty, id spelling agrees with `layoutPositions`); `e2e/dashboard.spec.ts` counts three on a real page    | unit, browser |
| the cascade is never overdrawn by the highlight   | `e2e/dashboard.spec.ts`: on the cascaded page every edge touching the table is the cascade's, so none is flowing, and the amber is untouched by the click                                 | browser       |
| reduced motion shows the finished panel           | `e2e/dashboard.spec.ts`: field opacity 1, and `animation-name: none` on both a sketch block and a flow edge                                                                               | browser       |
...
| the copy sweep's details exclusion is real        | `e2e/dashboard.spec.ts` now runs two of its states with a node pinned open. The exclusion existed for months against a panel that rendered no such label and a sweep that never opened it | browser       |
